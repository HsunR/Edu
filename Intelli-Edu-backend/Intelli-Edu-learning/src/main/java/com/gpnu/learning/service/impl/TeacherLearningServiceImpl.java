 ...
 * <p>
 * 注意：教师端的班级级聚合查询（statClassMastery / statFrequentQuestions /
 * statClassWrongByPoint / listWrongStudentsByQuestion / statWrongTypeByQuestion）
 * 在共享支撑类中没有对应的学生级方法，属于教师专属查询，直接走 mapper，不下沉到 LearningQuerySupport。
 * 只有“学生视角掌握度”与“知识点名称富化”这类共享逻辑才复用 LearningQuerySupport。
 */
@Service
public class TeacherLearningServiceImpl implements ITeacherLearningService {
    ...
    public List<MasteryOverviewVO> getStudentMastery(Long teacherId, Long classId, Long studentId) {
        ...
        return learningQuerySupport.listOverview(studentId, classId);
    }

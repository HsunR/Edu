    ...
    <el-card>
      ...
      <div class="filter-bar">
        ...
        <el-select v-model="filterStatus" placeholder="上传状态" clearable style="width: 120px" @change="handleSearch">
          <el-option label="成功" :value="UploadStatus.Success" />
          <el-option label="失败" :value="UploadStatus.Failed" />
          <el-option label="待确认" :value="UploadStatus.Pending" />
        </el-select>
        <el-date-picker
          v-model="dateRange"
          type="daterange"
        ...
        <el-table v-loading="loading" :data="resources" border style="width: 100%">
          ...
          <el-table-column prop="resourceType" label="类型" width="80" align="center">
            <template #default="{ row }">
              <el-tag size="small">{{ ResourceTypeLabels[row.resourceType] }}</el-tag>
            </template>
          </el-table-column>
      ...
      <div v-else class="card-grid">
        ...
        >
          <div class="card-preview">
            <img
              v-if="resource.resourceType === ResourceType.Image"
              :src="resource.accessUrl"
              :alt="resource.resourceName"
            />
            <div v-else-if="resource.resourceType === ResourceType.Video" class="card-icon video-icon">
              ▶
            </div>
          ...
          <div class="card-info">
            ...
            <div class="card-meta">
              <el-tag size="small">{{ ResourceTypeLabels[resource.resourceType as ResourceType] }}</el-tag>
